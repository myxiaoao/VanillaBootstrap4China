...
?>
<div id="DiscussionForm" class="Box">
	<h1><?php echo $this->Data('Title'); ?></h1>
	<?php
		echo $this->Form->Open(array('class' => 'form-horizontal'));
		...
		if ($Session->CheckPermission('Vanilla.Discussions.Announce'))
			$Options .= '<div class="checkbox">'.$this->Form->CheckBox('Announce', T('Announce'), array('value' => '1')).'</div>';

		if ($Session->CheckPermission('Vanilla.Discussions.Close'))
			$Options .= '<div class="checkbox">'.$this->Form->CheckBox('Closed', T('Close'), array('value' => '1')).'</div>';
		...
		echo Anchor(T('Cancel'), $CancelUrl, 'Cancel btn btn-danger');
		echo '</div>';
